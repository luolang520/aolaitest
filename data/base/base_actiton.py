...
class BaseAciton:
 ...
 #实现滑动业务
    ...
    #获取当前手机窗口大小
        ...
        width = screen_size.get("width")#获取手机的宽
        heigh = screen_size.get("height")#获取手机的高
        if tag == 1:#向上滚动
            self.driver.swipe(width * 0.5, heigh * 0.8, width * 0.5, heigh * 0.3, 1000)
        if tag == 2:#向下滚动
            self.driver.swipe(width * 0.5, heigh * 0.3, width * 0.5, heigh * 0.8, 1000)
        if tag == 3:#向左滚动
            self.driver.swipe(width * 0.8, heigh * 0.5, width * 0.3, heigh * 0.5, 1000)
        if tag == 4:#向右滚动
            self.driver.swipe(width * 0.3, heigh * 0.5, width * 0.8, heigh * 0.5, 1000)

    #获取吐司的消息
    def get_toast_message(self, message):
        toast_xpath = "//*[contains(@text,'{}')]".format(message)
        print("toast_xpath--->", toast_xpath, "message--->", message)
